...
function Experience(){
	const exps = personalInfo.experiences
...
function Job({job}: JobProps){
	...
	const tags = job.tags.map((tag, index) => {
		return (
			<li className="mr-1.5 mt-2" key={index}>
				<Badge variant="secondary" className="bg-teal-400/10 text-teal-300"> {tag} </Badge>
			</li>
		)
	})
	return (
